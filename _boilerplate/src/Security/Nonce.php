 ...
 *
 * @package LEAStudios\PluginName\Security
 */

declare(strict_types=1);

namespace LEAStudios\PluginName\Security;
...
class Nonce {
	...
	public static function create( string $action ): string {
		return wp_create_nonce( 'plugin_name_' . $action );
	}
	...
	public static function verify( string $nonce, string $action ): bool {
		return (bool) wp_verify_nonce( $nonce, 'plugin_name_' . $action );
	}
	...
	public static function check_request( string $action, string $param_key = '_wpnonce' ): void {
		check_admin_referer( 'plugin_name_' . $action, $param_key );
	}
	...
	public static function check_ajax( string $action, string $param_key = '_wpnonce' ): void {
		check_ajax_referer( 'plugin_name_' . $action, $param_key );
	}
}
